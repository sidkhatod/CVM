        ...
        for (char currCh : inProgram) {
            ...
            switch (currCh) {
                ...
                case '=':
                    if (currentToken.mType == STRING_LITERAL) {
                        currentToken.mText.append(1, currCh);
                    } else if (currentToken.mType == OPERATOR && currentToken.mText == "=") {
                        // previous char was '=', so this makes '=='
                        currentToken.mText.append(1, currCh);
                        endToken(currentToken, tokens);
                    } else {
                        // start a new '=' operator (don't emit yet, wait to see if next is also '=')
                        endToken(currentToken, tokens);
                        currentToken.mType = OPERATOR;
                        currentToken.mText.append(1, currCh);
                        // don't call endToken here — let next char decide
                    }
                    break;
