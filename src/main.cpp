...
static const char* sOpcodeNames[] = {
    "EXIT", "ADD_INT", "SUB_INT", "MUL_INT", "DIV_INT",
    "PUSH_INT", "POP_INT", "PRINT_INT", "COMP_INT_LT", "COMP_INT_EQ",
    "LOAD_INT", "STORE_INT", "JUMP_BY_IF_ZERO", "JUMP_BY",
    "LOAD_INT_BASEPOINTER_RELATIVE", "STORE_INT_BASEPOINTER_RELATIVE",
    "CALL", "RETURN", "READ_INT"
};

struct Parameter { string mName; size_t mIndex; };
struct CompiledFunction { size_t mInstructionOffset; size_t mNumArguments; bool mReturnsSomething; };

void GenerateCodeForStatement(const Statement& currStmt, const map<string,int16_t>& variableOffsets, map<string,Parameter> parameters, vector<int16_t>& returnCmdJumpInstructions, vector<Instruction>& compiledCode, map<string,CompiledFunction>& functionNameToInstruction);
void GenerateCodeForFunction(const FunctionDefinition& currFunc, vector<Instruction>& compiledCode, map<string,CompiledFunction>& functionNameToInstruction);
...
void GenerateCodeForStatement(const Statement& currStmt, const map<string,int16_t>& variableOffsets, map<string,Parameter> parameters, vector<int16_t>& returnCmdJumpInstructions, vector<Instruction>& compiledCode, map<string,CompiledFunction>& functionNameToInstruction)
{
    ...
    case StatementKind::VARIABLE_DECLARATION:
        if (currStmt.mType.mType == INT32 && !currStmt.mParameters.empty()) {
            ...
            if (init.mKind != StatementKind::LITERAL) {
                ...
                GenerateCodeForStatement(init, variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
    ...
    case StatementKind::FUNCTION_CALL:
        if (currStmt.mName == "return") {
            if (currStmt.mParameters.size() != 1) throw runtime_error("return expects one parameter.");
            ...
            compiledCode.push_back(Instruction{STORE_INT_BASEPOINTER_RELATIVE, 0, int16_t(-2-(int)parameters.size())});
            ...
            if (currStmt.mParameters.size() != 1) throw runtime_error("print expects one parameter.");
            GenerateCodeForStatement(currStmt.mParameters[0], variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
            compiledCode.push_back(Instruction{PRINT_INT, 0, 0});
        } else if (currStmt.mName == "input") {
            if (!currStmt.mParameters.empty()) throw runtime_error("input takes no parameters.");
            compiledCode.push_back(Instruction{READ_INT, 0, 0});
        } else {
            ...
            if (foundFn->second.mReturnsSomething)
                compiledCode.push_back(Instruction{PUSH_INT, 0, 0});
            ...
            for (auto& p : currStmt.mParameters)
                GenerateCodeForStatement(p, variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
    ...
    case StatementKind::LITERAL:
        if (currStmt.mType.mType == INT32)
            compiledCode.push_back(Instruction{PUSH_INT, 0, int16_t(stoi(currStmt.mName))});
        break;
    ...
    case StatementKind::OPERATOR_CALL:
        ...
        if (currStmt.mName == "=") {
            const string& varName = currStmt.mParameters[0].mName;
            auto foundVar = variableOffsets.find(varName);
            if (foundVar != variableOffsets.end()) {
                ...
                auto foundParam = parameters.find(varName);
                if (foundParam == parameters.end())
                    throw runtime_error(string("Unknown variable \"") + varName + "\".");
                GenerateCodeForStatement(currStmt.mParameters[1], variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
    ...
    case StatementKind::WHILE_LOOP: {
        ...
        for (auto it = currStmt.mParameters.begin()+1; it != currStmt.mParameters.end(); ++it)
            GenerateCodeForStatement(*it, variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
    ...
    case StatementKind::IF_STATEMENT: {
        ...
        if (hasElse) compiledCode.push_back(Instruction{JUMP_BY, 0, 0});
        compiledCode[jumpOverThenIdx].p2 = int16_t(compiledCode.size() - jumpOverThenIdx);
        if (hasElse) {
            GenerateCodeForStatement(currStmt.mParameters[2], variableOffsets, parameters, returnCmdJumpInstructions, compiledCode, functionNameToInstruction);
...
void GenerateCodeForFunction(const FunctionDefinition& currFunc, vector<Instruction>& compiledCode, map<string,CompiledFunction>& functionNameToInstruction)
{
    ...
    functionNameToInstruction[currFunc.mName] = CompiledFunction{compiledCode.size(), currFunc.mParameters.size(), currFunc.mReturnsSomething};
    ...
    for (auto& stmt : currFunc.mStatements)
        GenerateCodeForStatement(stmt, varOffsets, params, returnJumps, compiledCode, functionNameToInstruction);
...
void runSource(const string& source, bool debugMode) {
    ...
    if (debugMode) {
        ...
        for (size_t i = 0; i < compiledCode.size(); ++i)
            cout << "[" << i << "] " << sOpcodeNames[compiledCode[i].opcode] << " p2=" << compiledCode[i].p2 << "\n";
    }
    ...
    int16_t result = 0;
    vector<int16_t> mainArgs(foundMain->second.mNumArguments, 0);
    BytecodeInterpreter::Run(compiledCode.data() + foundMain->second.mInstructionOffset, mainArgs, &result);
    cout << "\nResult: " << result << endl;
}
...
int main(int argc, const char* argv[]) {
    ...
    if (argc >= 2 && string(argv[1]) != "--debug") {
        ...
        if (!file.is_open()) { cerr << "Error: Cannot open file '" << filename << "'\n"; return 1; }
        stringstream ss;
        ss << file.rdbuf();
        try { runSource(ss.str(), debugMode); }
    ...
    while (true) {
        ...
        if (line == "exit") break;
        if (line == "--debug") { debugMode = !debugMode; cout << "Debug mode " << (debugMode ? "ON" : "OFF") << "\n"; continue; }
        if (line.empty()) {
            if (!source.empty()) {
                try { runSource(source, debugMode); }
                catch (exception& e) { cerr << "Error: " << e.what() << "\n"; }
                source.clear();
            }
            continue;
