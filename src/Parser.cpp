...
namespace simpleparser {
    ...
    Parser::Parser() {
        mTypes["void"] = Type("void", VOID);
        mTypes["int"]  = Type("signed int", INT32);
        mTypes["let"]     = Type("signed int", INT32);
        mTypes["bool"]    = Type("signed int", INT32);
        mTypes["unsigned"] = Type("unsigned int", UINT32);
        mTypes["char"] = Type("signed char", INT8);
        mTypes["uint8_t"] = Type("uint8_t", INT8);
        mTypes["double"] = Type("double", DOUBLE);
    ...
    optional<Statement> Parser::expectOneValue() {
        ...
        if (mCurrentToken != mEndToken && mCurrentToken->mType == IDENTIFIER
            && (mCurrentToken->mText == "true" || mCurrentToken->mText == "false")) {
            Statement boolLiteral;
            boolLiteral.mKind = StatementKind::LITERAL;
            boolLiteral.mName = (mCurrentToken->mText == "true") ? "1" : "0";
            boolLiteral.mType = Type("signed int", INT32);
            result = boolLiteral;
            ++mCurrentToken;
            return result;
        }
    ...
    optional<Statement> Parser::expectWhileLoop() {
        ...
        if (!expectOperator("(")) {
            throw runtime_error(string("Expected '(' after 'while' on line ") + to_string(lineNo) + ".");
        }
        ...
        optional<Statement> condition = expectExpression();
        if (!condition) {
            throw runtime_error(string("Expected loop condition after 'while' on line ") + to_string(lineNo) + ".");
        }

        whileLoop.mParameters.push_back(condition.value());

        if (!expectOperator(")")) {
            throw runtime_error(string("Expected ')' after while condition on line ") + to_string(lineNo) + ".");
        }
        ...
        while (mCurrentToken != mEndToken && !expectOperator("}")) {
            ...
            if (!expectOperator(";").has_value()) {
                size_t ln = (mCurrentToken != mEndToken) ? mCurrentToken->mLineNumber : 999999;
                throw runtime_error(string("Expected ';' at end of statement in line ") + to_string(ln) + ".");
            }
        }
    ...
    optional<Statement> Parser::expectIfStatement() {
        size_t lineNo = (mCurrentToken != mEndToken) ? mCurrentToken->mLineNumber : SIZE_MAX;
        ...
        if (!expectOperator("("))
            throw runtime_error(string("Expected '(' after 'if' on line ") + to_string(lineNo) + ".");

        optional<Statement> condition = expectExpression();
        if (!condition)
            throw runtime_error(string("Expected condition after 'if(' on line ") + to_string(lineNo) + ".");

        if (!expectOperator(")"))
            throw runtime_error(string("Expected ')' after if-condition on line ") + to_string(lineNo) + ".");
        ...
        // Build then-block
        Statement thenBlock;
        thenBlock.mKind = StatementKind::BLOCK;
        thenBlock.mType = Type("void", VOID);

        while (mCurrentToken != mEndToken && !expectOperator("}")) {
            auto stmt = expectStatement();
            if (!stmt) break;
            thenBlock.mParameters.push_back(stmt.value());
            if (!expectOperator(";")) {
                size_t ln = (mCurrentToken != mEndToken) ? mCurrentToken->mLineNumber : 999999;
                throw runtime_error(string("Expected ';' after statement on line ") + to_string(ln) + ".");
            }
        }

        // Build else-block (may be empty)
        Statement elseBlock;
        elseBlock.mKind = StatementKind::BLOCK;
        elseBlock.mType = Type("void", VOID);

        if (expectIdentifier("else")) {
            if (!expectOperator("{"))
                throw runtime_error(string("Expected '{' after 'else' on line ") + to_string(lineNo) + ".");

            while (mCurrentToken != mEndToken && !expectOperator("}")) {
                auto stmt = expectStatement();
                if (!stmt) break;
                elseBlock.mParameters.push_back(stmt.value());
                if (!expectOperator(";")) {
                    size_t ln = (mCurrentToken != mEndToken) ? mCurrentToken->mLineNumber : 999999;
                    throw runtime_error(string("Expected ';' after statement on line ") + to_string(ln) + ".");
                }
        ...
        // Assemble IF_STATEMENT node:
        // mParameters[0] = condition
        // mParameters[1] = then-block
        // mParameters[2] = else-block
        Statement ifStmt;
        ifStmt.mKind = StatementKind::IF_STATEMENT;
        ifStmt.mType = Type("void", VOID);
        ifStmt.mParameters.push_back(condition.value());
        ifStmt.mParameters.push_back(thenBlock);
        ifStmt.mParameters.push_back(elseBlock);

        return ifStmt;
    }

    optional<Statement> Parser::expectStatement() {
        optional<Statement> result = expectIfStatement();
        if (!result.has_value()) {
            result = expectWhileLoop();
        }
        if (!result.has_value()) {
            result = expectVariableDeclaration();
        }
